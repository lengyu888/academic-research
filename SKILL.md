...
description: 学术科研全流程助手 v4.0，覆盖从选题→文献调研→实验设计→数据分析→公式推导→论文撰写→润色→投稿→代码发布→海报→PPT→学位论文的完整研究闭环（13个模块）。触发关键词包括论文、文献、综述、数据分析、公式推导、学术写作、科研报告、课程作业、实验报告、毕业设计、课题研究、文献调研、论文修改、学术英语、LaTeX排版、SPSS/Python数据分析、数学推导、PPT、答辩、开题、选题、实验设计、投稿、Cover Letter、审稿意见、rebuttal、被拒、改投、海报、墙报、README、代码文档、复现等。适用于本科生、研究生、博士生的全流程科研场景。
metadata:
...
3. **优先级**：当模块判断模糊时，优先考虑用户的核心意图：
   ...
   - 提到"润色""修改""语法""表达""去AI味""太AI" → `paper-polish`（默认保持原文语言）
   - 提到"翻译""translate""改成英文""用英文写" → `paper-polish`（改变输出语言）
   ...
   - 提到"被拒""改投""审稿意见修改""revision""rejection" → `paper-polish` + `paper-submission` 组合
...
- 保持客观、严谨的学术态度
- **语言保持**：默认输出与输入同语言（中文输入→中文输出，英文输入→英文输出）。润色是改善表达，不是翻译。仅当用户明确要求（"翻译""改成英文"）时才改变语言
- **语言检查**：当用户明确指定输出语言（如"全英文"）时，输出前扫描全文，确保无其他语言字符混入（中文人名/机构名/数据集名除外）
...
| 论文被拒改投 | `paper-polish` → `paper-submission` | 审稿意见修订 → 改投 Cover Letter（含修订历史） |
...
- 公式推导必须展示完整过程，不能跳步
- 数据分析必须说明方法和局限性
- 论文润色保持原意，不改变学术观点；默认保持原文语言，不自动翻译
...
| `analysis_report.py` | CSV → LaTeX 报告自动生成（含 `--figures` 图表生成） | data-processing，一键出 .tex + 图表 + PDF |
...
CSV 数据 → quick_stats.py（概览）→ dep_check.py（装依赖）→ 完整分析
         → analysis_report.py --figures --compile（一键 .tex + 图表 + PDF）
```
...
- "我的论文被 CVPR 拒了，审稿人说缺少对比实验，帮我修改后改投 ACM MM"
- "这段中文论文帮我润色一下，不要翻译成英文"
- "这个 CSV 有实验数据，帮我生成带图表的 PDF 报告"
