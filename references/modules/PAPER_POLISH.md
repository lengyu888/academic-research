...
#### ⚠️ 语言保持原则（Language Preservation）

**默认规则：输出语言 = 输入语言。润色是改善表达，不是翻译。**

| 输入语言 | 用户未指定语言时 | 用户要求"润色"时 | 用户要求"翻译成英文"时 |
|----------|-----------------|-----------------|----------------------|
| 中文 | 输出中文 | 输出中文 | 输出英文 |
| 英文 | 输出英文 | 输出英文 | 输出英文 |
| 中英混合 | 保持各自语言 | 保持各自语言 | 全部转英文 |

**判断方法**：通读全文，如果超过 70% 的内容是中文，则判定为中文论文，润色后必须保持中文。只有用户明确使用"翻译""translate""改成英文""用英文写"等表述时，才改变语言。

**常见错误**：将中文论文的 Abstract、Introduction 等章节自动翻译成英文，而 Methods、Results 保持中文——这会导致全文语言不一致。正确做法要么全部中文润色，要么全部英文（仅当用户要求时）。
...
## 审稿意见驱动修订（Revision Workflow）

当论文被拒或收到审稿意见需要修订时，使用以下工作流。此场景通常与 `paper-submission` 模块联动。

### 触发场景

- 论文被拒，需要修改后改投
- 收到 R&R（Revise and Resubmit）
- 审稿人要求补充实验、增加分析

### 修订流程

1. **解析审稿意见**：逐条提取审稿人的修改要求，分类为：
   - **实验补充类**：需要增加对比实验、消融实验、数据集
   - **方法完善类**：需要修正公式、补充解释、增加图示
   - **写作改进类**：语法错误、逻辑不清、表述模糊
   - **文献补充类**：需要引用特定论文或扩展相关工作

2. **逐条对照原文**：定位每条审稿意见对应的具体段落/表格/公式

3. **执行修改**：
   - 实验补充：在实验章节插入新实验，更新结果表格和分析
   - 方法完善：修订公式、补充推导步骤、增加架构图
   - 写作改进：按本模块的润色流程处理
   - 文献补充：在相关工作章节和引用列表中添加

4. **生成修订说明**：输出一份修改清单，记录每处修改的位置和原因

5. **联动 `paper-submission`**：生成 Response to Reviewers，逐条回复
...
### 修改清单
| # | 位置 | 原文 | 修改后 | 审稿意见对应 | 修改原因 |
|---|------|------|--------|-------------|----------|
| 1 | §4.2, Table 3 | — | 新增对比实验 | R1-Comment2 | 补充 SOTA 对比 |
| 2 | §5 | — | 新增可视化分析 | R2-Comment1 | 增强可解释性 |
...
## 语言强制检查（Language Enforcement）

**前提**：先执行"语言保持原则"——默认输出与输入同语言。只有当用户明确要求改变语言时，才执行以下检查。

当用户明确要求输出语言（如"全英文""全部用英文写"）时，必须执行语言检查。

### 检查规则

1. **英文输出检查**：
   - 扫描全文中文字符（Unicode 范围：`一`-`鿿`，含扩展区）
   - 发现中文 → 自动替换为英文等价表述
   - 例外：中文人名、机构名、数据集名（如 "BraTS""IEMOCAP"）可保留

2. **中文输出检查**：
   - 扫描是否有不必要的英文混入（专业术语除外）
   - 确保中文标点符号正确（句号用 `。` 而非 `.`）

3. **最终确认**：输出前做一次全文扫描，报告语言一致性结果
...
if cn:
    print(f'[WARN] Found {len(cn)} Chinese segments:')
    for s in cn[:10]: print(f'  - {s}')
else:
